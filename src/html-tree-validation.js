...
const disallowed_children = {
    ...
    select: {
        only: ["option", "optgroup", "#text", "hr", "script", "template"],
    },
 ...
 * @returns {string | null}
 */
export function is_tag_valid_with_ancestor(
    child_tag,
    ancestors,
    child_loc,
    ancestor_loc,
    ...
    if (
        "descendant" in disallowed &&
        disallowed.descendant.includes(child_tag)
    ) {
        const child = child_loc
            ? `\`<${child_tag}>\` (${child_loc})`
            : `\`<${child_tag}>\``;
 ...
 * @returns {string | null}
 */
export function is_tag_valid_with_parent(
    child_tag,
    parent_tag,
    child_loc,
    parent_loc,
