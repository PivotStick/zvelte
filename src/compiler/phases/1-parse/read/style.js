/** @import { Attribute, SpreadAttribute, Directive, Css } from '#ast' */
...
function read_body(parser, close) {
    ...
    throw parser.error(`Expected token ${close}`, parser.template.length);
}
...
function read_selector_list(parser, inside_pseudo_class = false) {
    ...
    throw parser.error("Unexpected end of input", parser.template.length);
}
...
function read_selector(parser, inside_pseudo_class = false) {
    ...
    while (parser.index < parser.template.length) {
        ...
        } else if (inside_pseudo_class && parser.matchRegex(REGEX_NTH_OF)) {
        ...
        } else if (parser.matchRegex(REGEX_PERCENTAGE)) {
            relative_selector.selectors.push({
                type: "Percentage",
        ...
        } else if (!parser.matchRegex(REGEX_COMBINATOR)) {
        ...
        if (
            parser.match(",") ||
            (inside_pseudo_class ? parser.match(")") : parser.match("{"))
        ) {
        ...
        if (combinator) {
            if (relative_selector.selectors.length === 0) {
                if (!inside_pseudo_class) {
                    throw parser.error(`Invalid selector`, start);
                }
            } else {
                relative_selector.end = index;
                children.push(relative_selector);
            ...
            if (
                ...
                throw parser.error(`Invalid selector`, parser.index);
            }
        }
    }
...
function read_declaration(parser) {
    ...
    const property = parser.readUntil(REGEX_WHITESPACE_OR_COLON);
    parser.allowWhitespace();
    parser.eat(":");
    ...
    if (!value && !property.startsWith("--")) {
        throw parser.error("Declaration cannot be empty", start);
    }
...
function read_attribute_value(parser) {
    ...
    while (parser.index < parser.template.length) {
        ...
        if (escaped) {
            ...
            quote_mark ? char === quote_mark : REGEX_CLOSING_BRACKET.test(char)
        ) {
            if (quote_mark) {
...
function read_identifier(parser) {
    ...
    if (
        parser.match("--") ||
        parser.matchRegex(REGEX_LEADING_HYPHEN_OR_DIGIT)
    ) {
        throw parser.error(
            `Expected a valid CSS identifier -->> "${parser.template.slice(parser.index, parser.index + 10)}..." `,
            start,
        );
    }
    ...
    if (identifier === "") {
        throw parser.error("Expected a valid CSS identifier", start);
    }
...
function allow_comment_or_whitespace(parser) {
    ...
    while (parser.match("/*") || parser.match("<!--")) {
        if (parser.eat("/*")) {
            parser.readUntil(REGEX_COMMENT_CLOSE);
            parser.eat("*/", true);
        }

        if (parser.eat("<!--")) {
            parser.readUntil(REGEX_HTML_COMMENT_CLOSE);
            parser.eat("-->", true);
        }
