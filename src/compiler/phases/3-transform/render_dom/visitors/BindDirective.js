...
/**
 * @param {import('#ast').BindDirective} node
...
export function BindDirective(node, context) {
    ...
    if (property?.event) {
        ...
        switch (node.name) {
            ...
            case "group": {
                const indexes = node.metadata.parent_each_blocks.map(
                    (/** @type {any} */ each) => {
                ...
                if (parent?.type === "RegularElement") {
                    ...
                    if (value !== undefined) {
                        group_getter = b.thunk(
                            b.block([
                                b.stmt(
                                    build_attribute_value(value, context).value,
                                ),
                                b.return(
    ...
    if (node.name === "this") {
        ...
        context.state.after_update.push(
            b.stmt(
                has_action_directive ? b.call("$.effect", b.thunk(call)) : call,
            ),
        );
    }
}
