...
export function RegularElement(node, context) {
    ...
    for (const attribute of other_directives) {
        if (attribute.type === "OnDirective") {
            ...
            if (has_use) {
                element_state.init.push(
                    b.stmt(b.call("$.effect", b.thunk(handler))),
                );
            } else {
    ...
    if (has_spread) {
        ...
        for (const attribute of /** @type {AST.Attribute[]} */ (attributes)) {
            ...
            if (
                ...
                let value = is_text_attribute(attribute)
                    ? attribute.value[0].data
                    : true;

                if (
                    name === "class" &&
                    node.metadata.scoped &&
                ...
                context.state.init.push(
                    b.stmt(b.call("$.autofocus", node_id, value)),
                );
            } else if (name === "class") {
                const is_html =
                    context.state.metadata.namespace === "html" &&
    ...
    const use_text_content =
        ...
        trimmed.every(
            (node) =>
                node.type === "Text" || !node.metadata.expression.has_state,
        ) &&
        ...
        context.state.init.push(
            b.block([
                ...
                child_state.update.length > 0
                    ? build_render_statement(child_state)
                    : b.empty,
...
function build_custom_element_attribute_update_assignment(
    ...
    const update = has_state
        ? b.call("$.template_effect", b.thunk(call))
        : call;
...
export function build_class_directives_object(class_directives, context) {
    ...
    for (const d of class_directives) {
        ...
        has_call_or_state ||=
            d.metadata.expression.has_call || d.metadata.expression.has_state;
    }

    const directives = b.object(properties);

    return has_call_or_state
        ? get_expression_id(context.state, directives)
        : directives;
}
