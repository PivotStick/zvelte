...
const analysis_visitors = {
    ...
    RelativeSelector(node, context) {
        ...
        node.metadata.is_global_like ||= !!node.selectors.find(
            (child) =>
                child.type === "PseudoClassSelector" && child.name === "root",
        );
    ...
    Rule(node, context) {
        ...
        node.metadata.is_global_block = node.prelude.children.some(
            (selector) => {
        ...
        node.metadata.has_local_selectors = node.prelude.children.some(
            (selector) => {
                return selector.children.some(
                    ({ metadata }) =>
                        !metadata.is_global && !metadata.is_global_like,
                );
            },
        );
    },
...
const validation_visitors = {
    Rule(node, context) {
        if (node.metadata.is_global_block) {
            ...
            const relative_selector =
                complex_selector.children[complex_selector.children.length - 1];
            ...
            if (
                ...
                throw new Error(
                    `A :global {...} block cannot follow a ${relative_selector.combinator.name} combinator`,
                );
            ...
            const declaration = node.block.children.find(
            ...
            if (a !== b) {
                for (let i = a; i <= b; i += 1) {
                    if (is_global(node.children[i])) {
                        ...
                        // e.css_global_invalid_placement(
                        //     node.children[i].selectors[0],
                        // );
                    }
                }
            }
        ...
        for (const relative_selector of node.children) {
            for (let i = 0; i < relative_selector.selectors.length; i++) {
                ...
                if (
                    ...
                    if (
                        relative_selector.selectors[i + 1]?.type ===
                        "TypeSelector"
                    ) {
                        throw new Error(
                            ":global(...) must not be followed with a type selector",
                        );
                        //     e.css_type_selector_invalid_placement(
                        //         relative_selector.selectors[i + 1],
                        //     );
                    }
    ...
    NestingSelector(_node, context) {
        const rule = /** @type {Css.Rule} */ (context.state.rule);
        if (!rule.metadata.parent_rule) {
            throw new Error("Nesting selectors can only be used inside a rule");
...
export function analyze_css(stylesheet, analysis) {
    walk(
        stylesheet,
        { keyframes: analysis.css?.keyframes ?? [], rule: null },
        css_visitors,
    );
}
