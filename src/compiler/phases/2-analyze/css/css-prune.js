 ...
 *   element: Compiler.RegularElement | Compiler.ZvelteElement;
 * }} State
 */
 ...
 * @param {Compiler.RegularElement | Compiler.ZvelteElement} element
 */
...
function apply_selector(relative_selectors, rule, element, stylesheet) {
    ...
    if (relative_selector.combinator) {
        ...
        switch (name) {
            case " ":
            case ">": {
                let parent = /** @type {Compiler.TemplateNode | null} */ (
                    // @ts-ignore
                    element.parent
                );
                ...
                return (
                    parent_matched ||
                    parent_selectors.every((selector) =>
...
function is_global(selector, rule) {
    ...
    for (const s of selector.selectors) {
        ...
        const has_global_selectors = selector_list?.children.some(
            (complex_selector) => {
...
function relative_selector_might_apply_to_node(
    ...
    for (const selector of relative_selector.selectors) {
        ...
        const name = selector.name.replace(
            regex_backslash_and_following_character,
            "$1",
        );
        ...
        switch (selector.type) {
            case "PseudoClassSelector": {
                ...
                if (
                    ...
                    const args = /** @type {Compiler.Css.SelectorList} */ (
                        selector.args
                    );
                    const complex_selector = args.children[0];
                    return apply_selector(
                        complex_selector.children,
                        rule,
                        element,
                        stylesheet,
                    );
            ...
            case "NestingSelector": {
                ...
                const parent = /** @type {Compiler.Css.Rule} */ (
                    rule.metadata.parent_rule
                );
        ...
        for (const chunk of chunks) {
            ...
            if (prev_values.length > 0) {
                ...
                current_possible_values.forEach((current_possible_value) => {
                    if (
                        regex_starts_with_whitespace.test(
                            current_possible_value,
                        )
                    ) {
...
/**
 * @param {Compiler.RegularElement | Compiler.ZvelteElement} node
 * @returns {Compiler.RegularElement | Compiler.ZvelteElement | null}
 */
function get_element_parent(node) {
    /** @type {Compiler.ZvelteNode | null} */
    let parent = node;
    while (
 ...
 *
 * The Zvelte `<slot>` is just a placeholder and is not actually real. Any children nodes
 ...
 * @returns {Map<Compiler.RegularElement | Compiler.ZvelteElement | Compiler.RenderTag, NodeExistsValue>}
 */
function get_possible_element_siblings(node, adjacent_only) {
    /** @type {Map<Compiler.RegularElement | Compiler.ZvelteElement | Compiler.RenderTag, NodeExistsValue>} */
    const result = new Map();

    /** @type {Compiler.ZvelteNode} */
    let prev = node;
    while ((prev = find_previous_sibling(prev))) {
        if (prev.type === "RegularElement") {
            ...
            const possible_last_child = get_possible_last_child(
                prev,
                adjacent_only,
            );
        ...
        } else if (prev.type === "RenderTag" || prev.type === "ZvelteElement") {
    ...
    if (!prev || !adjacent_only) {
        ...
        while (
            ...
            if (
                parent.type === "ForBlock" &&
                // @ts-expect-error
                !parent.fallback?.nodes.includes(node)
            ) {
                // `{#each ...}<a /><b />{/each}` — `<b>` can be previous sibling of `<a />`
                add_to_map(
                    get_possible_last_child(parent, adjacent_only),
                    result,
                );
            }
...
/**
 * @param {Compiler.ForBlock | Compiler.IfBlock | Compiler.AwaitBlock} relative_selector
 * @param {boolean} adjacent_only
...
function get_possible_last_child(relative_selector, adjacent_only) {
    ...
    /** @type {NodeMap} */
    const result = new Map();
    if (relative_selector.type === "ForBlock") {
        /** @type {NodeMap} */
        const each_result = loop_child(
            relative_selector.body.nodes,
            adjacent_only,
        );
        ...
        /** @type {NodeMap} */
        const if_result = loop_child(
            relative_selector.consequent.nodes,
            adjacent_only,
        );
        ...
        const not_exhaustive =
            !has_definite_elements(if_result) ||
            !has_definite_elements(else_result);
        if (not_exhaustive) {
...
/**
 * @param {Compiler.ZvelteNode[]} children
 * @param {boolean} adjacent_only
 */
